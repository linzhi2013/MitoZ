...
class CircularDrawer(AbstractDrawer):
        ...
        if track.axis_labels:
            for set in track.get_sets():
                if set.__class__ is GraphSet:
                    # Y-axis
                    # range(7) to range(3), changed by mengguanliang on 20160806 for testing.
                    for n in range(1):
                        ## angle = n * 1.0471975511965976, by mgl
                        ...
                        for graph in set.get_graphs():
                            ...
                            if graph.center is None:
                                midval = (maxval + minval)/2.
                                ## graph_label_min.append("%.3f" % minval) , by mgl
                                graph_label_min.append("%.0f" % minval)
                                ## graph_label_max.append("%.3f" % maxval), by mgl 
                                graph_label_max.append("%.0f" % maxval)
                                ## graph_label_mid.append("%.3f" % midval), by mgl 
                                graph_label_mid.append("%.0f" % midval)
                            else:
                        ...
                        for limit, x, y, in [(graph_label_min, x0, y0-5), ## (graph_label_min, x0, y0), by mgl
                                             (graph_label_max, x1, y1+2), ## (graph_label_max, x1, y1), by mgl ;(graph_label_mid, xmid, ymid), by mgl
                                             (graph_label_mid, xmid, ymid+3)]:
                            ...
                            # label.textAnchor = 'middle', by mgl
                            labelgroup = Group(label)
    ...
    def draw_greytrack(self, track):
        ...
        if track.greytrack_labels:
            ...
            labelstep = self.length//(track.greytrack_labels + 1) # label interval
            #for pos in range(self.start, self.end, labelstep):, by mgl
            for pos in range(self.start, self.end - labelstep, labelstep):
                ...
                # x, y = self.xcenter+btm*sintheta, self.ycenter+btm*costheta, by mgl
                x, y = self.xcenter+btm*sintheta - 18, self.ycenter+btm*costheta - 14  # start text halfway up marker
                # following two lines added by mgl
                #if track.name not in ['GC content', 'Coverage']:
                #    y -= 220

                labelgroup = Group(label)
                
                # labelangle = self.sweep*2*pi*(pos-self.start)/self.length - pi/2, by mgl
                labelangle = self.sweep*2*pi*(pos-self.start)/self.length - pi/2 + 1.55
                if theta > pi:
